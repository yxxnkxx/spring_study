...
public class QuerydslBasicTest {
    ...
    @Test
    public void concat() {
        List<String> fetch = queryFactory.select(member.username.concat("_").concat(member.age.stringValue())).from(member).where(member.username.eq("member1")).fetch();
        for (String s : fetch) {
            System.out.println("s = " + s);
        }
    }
    ...
    public void bulkUpdate() {
        ...
        for (Member member1 : result) {
            System.out.println("member1 = " + member1); // em.flush() 안 하면 비회원 처리 X
        }
    }
